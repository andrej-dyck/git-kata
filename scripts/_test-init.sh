...
check-init-script-file-mode() {
  ...
  [[ "$lsOutput" =~ ^100755\ .+ ]] || fail-check "Expected execution flag (+x '100755') for 'init.sh'" "Git ls-files was: $lsOutput" "Use 'git update-index --chmod=+x $EXERCISE_DIR_ARG/init.sh' to make 'init.sh' executable"
}
...
check-readme() {
  ...
  local todoMarkers=()
  mapfile -t todoMarkers < <(grep -n -E 'TODO|🚧 WIP' README.md || true)
  [[ ${#todoMarkers[@]} -eq 0 ]] || fail-check "Expected 'README.md' to contain no TODOs" "${todoMarkers[@]}"
}
